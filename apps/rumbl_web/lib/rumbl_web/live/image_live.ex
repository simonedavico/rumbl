defmodule RumblWeb.ImageLive do
  ...
  def render(assigns) do
    ...
    <div style="margin-left: <%= @depth * 50 %>px;">
      <form phx-change="update">
        ...
        <%= @width %>px
        <fieldset>
          <%= radio_tag(name: :bg, value: "white", checked: @bg) %> White
          <%= radio_tag(name: :bg, value: "black", checked: @bg) %> Black
          <%= radio_tag(name: :bg, value: "blue", checked: @bg) %> Blue
        </fieldset>
      </form>
      <br/>
      <img phx-click="boom" src="<%= Routes.static_path(RumblWeb.Endpoint, "/images/phoenix.png") %>" width="<%= @width %>" style="background: <%= @bg %>;" />
    </div>
    """
  end
